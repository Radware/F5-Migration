...
import re, os, datetime, tarfile, sys
try:
	from app import app
...
def fun_f5_mig( filename, project_name, mode):
			...
			for line in ''.join(node[:-1]).splitlines():
				if "ltm node" in line:
					if '/' in line:
						...
						name=line.replace('ltm node ', '').split(' ')[0]
						rd='Common'
					nodeDict.update({name: {}})
				elif "address" in line:
					junk, ip=line.split('address ')
					nodeDict[name].update({'rip': ip})
				elif "description" in line:
					nodeDict[name].update({'name': line.replace('    description ','').replace('}', '')})
				elif '}' == line:
					pass
					...
					if '/' in line:
						...
						junk, hc = line.replace('  ', '').replace('monitor ', '').split(' ')

					hc, log_write=fun_hc_long_name(hc, name, log_write)
					nodeDict[name].update({'health': hc})
				else:
					log_unhandeled.append(' Object type: Real\n Object name: ' + name + '\nLine: ' + line.replace(' ', ''))
			if rd!='Common':
				...
				for line in ''.join(member[:-1]).splitlines():
					...
					if line.replace(' ','')[0:7] == 'monitor':
						...
						if mNamePort.split(':')[0] in nodeDict:
							if not 'health' in nodeDict[mNamePort.split(':')[0]]:
								nodeDict[mNamePort.split(':')[0]].update({'health': mon})
							elif not nodeDict[mNamePort.split(':')[0]]['health']==mon:
								new_mNamePort=mNamePort.replace(mNamePort.split(':')[0], mNamePort.split(':')[0]+'_'+name)
					...
					if len(tmp_list)==1:
						...
						for member in list(new_group['members']):
							if member in tmp_dict and tmp_dict[member]==max_val:
								...
								print('Unhandled exception in priority-group to backup convertion! Please check manually')
								# print(member)
		...
		for persist in re.findall('(^ltm persistence.+{\n(  .+\n)+^})', text, re.MULTILINE):
			for line in ''.join(persist[:-1]).splitlines():
				...
				if "ltm persistence" in line:
					...
					persistDict.update({name: {'type':persistType}})
				elif 'timeout' in line:
					persistDict[name].update({'timeout':int(int(line.replace('timeout ',''))/60)})
				elif 'cookie-name' in line:
					persistDict[name].update({'cookie-name':line.replace('cookie-name ','')})
				elif 'method' in line:
					persistDict[name].update({'method':line.replace('method ','')})
	...
	def virtParser(text):
		...
		for virt in re.findall('(^ltm virt.+{\n(  .+\n)+^})', text, re.MULTILINE):
			...
			virtDict.update({name: {'adv': {} ,'service': { },'profiles': {}}})

			strProf=''
			for prof in re.findall('^(    profiles.+{\n(.+\n)+.+}\n    }\n|^    profiles.+{\n.+}\n    }\n)', strVirt, re.MULTILINE):
				strProf=''.join(prof[:-1])
			...
			if '    source-address-translation {' in strVirt:
				...
				for line in strSnat.replace('  ','').splitlines()[1:-1]:
					...
					if x == 'type':
						...
						virtDict[name]['service'].update({'pip addr': y})
			...
			strRules=''
			if '    rules {' in strVirt:
				strRules=re.search(r'(.+rules \{(.*\n)+    \})', strVirt).group(0)
				if len(re.findall('}', strRules))>1:
					strRules=strRules[:strRules.index('    }')+5]
				## Need to complete iRules!!
			...
			strVirt = strVirt.replace(strPersist, '').replace(strSnat[:-1], '').replace(strProf, '').replace(strRules,'').replace(strVirtVlan, '')
			strProf=re.sub(r'        (.+)\{\n            (.+)\n        \}', r'\1#=#\2',strProf).replace('  ','').replace('profiles {\n','')
			strProf=strProf.replace(' #=#context ', '#=#').replace(' { }', '#=#general')
			strPersist=re.sub(r'        (.+)\{\n            (.+)\n        \}', r'\1#=#\2',strPersist).replace('  ','').replace('persist {\n','')
			...
			for line in strVirt.splitlines():
				if 'mask' in line:
					if line.split('    mask ')[1]!='255.255.255.255':
						...
						delVirt=1
				elif 'ltm virtual' in line or line.replace('  ','') in ['}', '', 'translate-address enabled', 'translate-port enabled']:
					continue
				elif 'destination' in line:
					if '/' in line:
					...
					virtDict[name].update({'vip': vip, 'dport': dport})
					# VIP + PORT
				elif 'source ' in line:
					...
					if source=='0.0.0.0/0':
						...
						virtDict[name].update({'source': source})
				elif 'ip-protocol' in line:
					virtDict[name].update({'proto': ''.join(line.split('    ip-protocol '))})
				elif '    pool ' in line:
					if '/' in line:
						virtDict[name]['service'].update({'group': (''.join(line.split('    pool ')).split('/')[2])})
					else:
						virtDict[name]['service'].update({'group': line.replace('  ','').split(' ')[1]})
				elif 'description' in line:
					virtDict[name].update({'name': ''.join(line.replace('  ','').split('description '))})
				elif 'mirror' in line:
					virtDict[name]['service'].update({'mirror': 'ena'})
				elif 'vs-index' in line:
					continue
				elif line == '    disabled':
					virtDict[name]['service'].update({'disable': ' '})
				else:
					log_unhandeled.append(' Object type: Virt\n Object name: %s \n Line: %s\n' % (name, line.replace('  ', '')))
			for line in strProf.splitlines():
				if '/' in line:
					junk,rd, prof=line.split('/')
				elif '}' == line:
					continue
				else:
					prof=line
				...
				virtDict[name]['profiles'].update({profName:{'type': profType, 'context':context}})
				...
				if profType == 'http' and aplic=='basic-slb':
					...
					aplic='https'
				virtDict[name].update({'aplic': aplic})
			...
			for line in strPersist.splitlines():
				if '/' in line:
					junk, rd, persistName = line.split('/')
				else:
					persistName = line
				persistName, atrib = persistName.split('#=#')
				if atrib=='default yes':
					if persistName in persistDict:
						virtDict[name]['persist']=persistDict[persistName]
					elif persistName in defPersistDict:
						virtDict[name]['persist']=defPersistDict[persistName]
					else:
	...
	def vlanParser(text):
		...
		for vlan in re.findall('(^net vlan .+{\n(  .+\n)+^})', text, re.MULTILINE):
			...
			ifList=[]
			tmpDict={}
			for line in strVlan.splitlines():
				if "net vlan" in line:
					if '/' in line :
			...
			try:
				inter=''.join(strVlan[strVlan.index('  interfaces {'):strVlan.index('\n    }\n', strVlan.index('  interfaces {'))].splitlines()[1:])
				...
				for port in fun_loop_mult_val(inter):
					...
					if port[0] in trunk_dict:
						ifList.extend(trunk_dict[port[0]]['members'])
					elif port[0] in lacp_dict:
						ifList.extend(lacp_dict[port[0]]['port'])
					else:
						ifList.append(port[0])
		...
		return log_write,log_unhandeled
			...
			#for ifs in re.findall('^( {4}interfaces.+)', strVlan, re.MULTILINE):
			#	ifs=''.join(ifs)
			#	#print('was'+ifs)
			#	if "tagged" in ifs:
	...
	def mng_parser(text):
		...
		try:
			...
			mng_ip, mng_mask=re.search(r'(sys management-ip (.+) { })', text).group(0).replace('sys management-ip ','').split('/')
		...
		for mng_route in re.findall('(^sys management-route.+{\n(  .+\n)+^})', text, re.MULTILINE):
			for line in ''.join(mng_route[:-1]).splitlines():
				...
				if "sys management-route" in line:
					...
					log_unhandeled.append(' Object type: Route \n Object name: '+name+'\n Line: '+line.replace('  ',''))
			if net=='default':
		...
		try:
			tmp_dict = {}
			for line in re.sub(r'( +)}\n( )+}\n}', r'\1}', re.search(r'(sys syslog.+\n( .+\n)+})', text).group(0).replace('sys syslog {', '').replace('remote-servers {', '')).replace('  ','').splitlines():
				if '}' in line:
					tmp+=line+'\n'
				else:
					tmp += ' '+line
					...
					for comm in snmp[snmp.index('communities')+14:snmp.index('\n    }\n', snmp.index('communities'))].replace('  ','').replace('\n','##=##').split('}'):
						...
						for tmp_atrib in list(filter(None, comm_atrib.split('##=##'))):
							...
							if key == 'community-name':
								...
								log_unhandeled('Object type: SNMP Config \n Object name: '+comm_id+'\n Line :'+tmp_atrib)
	...
	for x in virtDict:
		...
		if virtDict[x]['vip']=='0.0.0.0':
			continue
		return_string+=('/c/slb/virt %s\n    ena\n    ipver v4\n    vip %s\n' % (x,virtDict[x]['vip']))
		out.write ('/c/slb/virt %s\n    ena\n    ipver v4\n    vip %s\n' % (x,virtDict[x]['vip']))
		if 'name' in virtDict[x]:
			#print('    vname '+virtDict[x]['name'])
			return_string+=('    vname %s\n' % virtDict[x]['name'])
			out.write ('    vname %s\n' % virtDict[x]['name'])
		#print('/c/slb/virt %s/service %s %s' % (x, virtDict[x]['dport'], virtDict[x]['aplic']))
		return_string+=('/c/slb/virt %s/service %s %s\n' % (x, virtDict[x]['dport'], virtDict[x]['aplic']))
		out.write ('/c/slb/virt %s/service %s %s\n' % (x, virtDict[x]['dport'], virtDict[x]['aplic']))
		for y in virtDict[x]['service']:
			#print('    '+y+' '+virtDict[x]['service'][y])
			return_string+=('    %s %s\n' % (y, virtDict[x]['service'][y]))
			out.write ('    %s %s\n' % (y, virtDict[x]['service'][y]))
		if 'persist' in virtDict[x]:
			#print(virtDict[x]['persist'])
			if virtDict[x]['persist']['type']=='cookie':
				if 'method' in virtDict[x]['persist']:
					cMethod=virtDict[x]['persist']['method']
				else:
					cMethod='insert'
				
				if 'cookie-name' in virtDict[x]['persist']:
					cName='"'+virtDict[x]['persist']['cookie-name']+'"'
				else:
					cName='"MyPersistCookie"'

				#print('pbind %s %s %s 10 10' % (virtDict[x]['persist']['type'], cMethod, cName))
				return_string+=('    pbind %s %s %s 10 10\n' % (virtDict[x]['persist']['type'], cMethod, cName))
				out.write ('    pbind %s %s %s 10 10\n' % (virtDict[x]['persist']['type'], cMethod, cName))
			elif virtDict[x]['persist']['type']=='clientip':
				if 'timeout' in virtDict[x]['persist']:
					timeout=virtDict[x]['persist']['timeout']
				else:
					timeout='10'
				return_string+=('    pbind %s\n    ptmout %s\n' % (virtDict[x]['persist']['type'], timeout))
				out.write ('    pbind %s\n    ptmout %s\n' % (virtDict[x]['persist']['type'], timeout))
			elif virtDict[x]['persist']['type']=='ssl':
				if 'timeout' in virtDict[x]['persist']:
					timeout=virtDict[x]['persist']['timeout']
				else:
					timeout='10'
				...
				log1str+=(' Object type: Virt \n Object name: %s \n Line: %s' % (x, virtDict[x]['persist']))
				log1.write('\n Object type: Virt \n Object name: %s \n Line: %s\n' % (x, virtDict[x]['persist']))
		for y in virtDict[x]['profiles']:
			tmpType=virtDict[x]['profiles'][y]['type']
			if tmpType=='one-connect' and virtDict[x]['aplic'] in ['http', 'https']:
				return_string+=('    http\n    connmgt ena 10\n..\n')
				out.write ('    http\n    connmgt ena 10\n..\n')
			elif tmpType=='one-connect':
				return_string+=('    connmgt ena 10\n')
				out.write ('    connmgt ena 10\n')
		for z in virtDict[x]['adv']:
		...
		if 'ssl' in virtDict[x]:
			if 'be' in virtDict[x]['ssl'] and 'fe' in virtDict[x]['ssl']:
				return_string+=('/c/slb/ssl/sslpol %s/fessl e/backend/ssl e\n/c/slb/virt %s/service %s %s/ssl/sslpol %s\n' % (x[:32], x, virtDict[x]['dport'], virtDict[x]['aplic'],x[:32]))
				out.write ('/c/slb/ssl/sslpol %s/fessl e/backend/ssl e\n/c/slb/virt %s/service %s %s/ssl/sslpol %s\n' % (x[:32], x, virtDict[x]['dport'], virtDict[x]['aplic'],x[:32]))
			elif 'fe' in virtDict[x]['ssl']:
				return_string+=('/c/slb/ssl/sslpol %s/fessl e/backend/ssl d\n/c/slb/virt %s/service %s %s/ssl/sslpol %s\n' % (x[:32], x, virtDict[x]['dport'], virtDict[x]['aplic'],x[:32]))
	...
	if virtDict == {} and poolDict == {} and nodeDict == {} and monitorDict == {} and ifDict == {} and floatIfDict == {} and vlanDict== {} :
